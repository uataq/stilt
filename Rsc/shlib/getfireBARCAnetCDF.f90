...
!---------------------------------------------------------------------------------------------------
   ...
   ncstat = NF90_INQ_VARID(ncid(fi), 'CO.emission', varid(fi))
   CALL nc_handle_err (ncstat)
...
SUBROUTINE getfireBARCAnetCDF (fname, n, date, i, j, ires, jres, f)
...
!-------------------------------------------------------------------------------
   ...
   INTEGER, PARAMETER :: imax=220, jmax=200                 ! horiz. field dimensions in file
   INTEGER            :: fi, k, l,                   &
                         i1(n), i2(n), j1(n), j2(n), &      ! field boundaries to read from file
                         dayofyear                          ! day # in year
...
!---------------------------------------------------------------------------------------------------
   ...
   DO k=1,n
      ...
      IF (i1(k) < 1 .OR. i1(k) > imax .OR. j1(k) < 1 .OR. j1(k) > jmax) THEN
         PRINT '(a,i0)'      , 'Subroutine getfireBARCAnetCDF: Field index exceedance at input item ', k
...
END SUBROUTINE getfireBARCAnetCDF
