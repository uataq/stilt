...
subpart<-part[(part[,"btime"]>(foottimes[foottimespos]))&(part[,"btime"]<=(foottimes[foottimespos+1])),]
...
max.x<-subpart[order(subpart[,"btime"],subpart[,"gitx"]),][selast>0,"gitx"]
min.x<-subpart[order(subpart[,"btime"],subpart[,"gitx"]),][selfirst>0,"gitx"]
max.y<-subpart[order(subpart[,"btime"],subpart[,"gity"]),][selast>0,"gity"]
min.y<-subpart[order(subpart[,"btime"],subpart[,"gity"]),][selfirst>0,"gity"]
btime<-subpart[order(subpart[,"btime"],subpart[,"gity"]),][selfirst>0,"btime"]
...
for (name in unique(emissname)){
	...
	if(!is.null(fluxweighting))emissgridwt<-getr(paste(veghead,fluxweight,".",name,sep=""),path=vegpath) #get CO emission grid
        ...
        if (gridname>3|xpart==0|ypart==0){
                ...
                x<-ceiling((x-ceiling((numpix.x/4)*(xpart-1)))/shrink.x)
                y<-ceiling((y-ceiling((numpix.y/4)*(ypart-1)))/shrink.y)
        }
...
#now need to vary each x and y between coarse grid cells to map it on fine grid cell
		...
		yfine<-floor(yxg/1000)
			...
			xfine<-(xfine-1)*shrink.x+1
			yfine<-(yfine-1)*shrink.y+1
			xfine<-xfine+ceiling(numpix.x/4)*(xpart-1)#shift number to sub grid position
			yfine<-yfine+ceiling(numpix.y/4)*(ypart-1)
			xfine<-xfine:(xfine+shrink.x-1)
			yfine<-yfine:(yfine+shrink.y-1)
		}
