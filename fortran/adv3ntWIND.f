...
!dwen(20090810) SUBROUTINE ADV3NT(S,X1,Y1,ZX,SS,GLOBAL,NXP,NYP)
...
! JCL:save original vertical index
      ZNEW = ZX
!dwen(20090825)      ZNEW = MAX(1d0,ZNEW)
      ZNEW = MAX(1.0,ZNEW)
!****************************
...
!  IF(GLOBAL.AND.I1P.GT.NXP)I1P=1
  ...
  if (i1p > size(s,1) .or. j1p > size(s,2)) then
     write (*,*) 'subroutine adv3ntWIND: i1p and/or j1p exceeds allowable limit:', &
