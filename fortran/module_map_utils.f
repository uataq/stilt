...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   TYPE proj_info
      ...
      sequence                     ! needed for compatibility with module_defgrid: gset
...
!      END IF
   ...
   subroutine get_map_factor(proj, xlat_arr, mapfac_arr, start_mem_i, &
                             start_mem_j, end_mem_i, end_mem_j)

     ! adapted from process_tile_module.f90
      ...
      if (proj%code == PROJ_LC) then
         if (proj%truelat1 /= proj%truelat2) then
            colat1 = rad_per_deg*(90.0 - proj%truelat1)
            colat2 = rad_per_deg*(90.0 - proj%truelat2)
            ...
            do i=start_mem_i, end_mem_i
               do j=start_mem_j, end_mem_j
                  ...
                  mapfac_arr(i,j) = sin(colat2)/sin(colat) * &
                       & (tan(colat/2.0)/tan(colat2/2.0))**n
               end do
            end do
     
         else
            colat0 = rad_per_deg*(90.0 - proj%truelat1)
      
            do i=start_mem_i, end_mem_i
               do j=start_mem_j, end_mem_j
                  colat = rad_per_deg*(90.0 - xlat_arr(i,j))
                  mapfac_arr(i,j) = sin(colat0)/sin(colat)*(tan(colat/2.0) / &
      ...
      else if (proj%code == PROJ_PS) then
    
         do i=start_mem_i, end_mem_i
            do j=start_mem_j, end_mem_j
               mapfac_arr(i,j) = (1.0 + sin(rad_per_deg*abs(proj%truelat1))) / &
                    & (1.0 + sin(rad_per_deg*sign(1.,proj%truelat1)*xlat_arr(i,j)))
            end do
         end do
    
      ! Mercator projection 
      else if (proj%code == PROJ_MERC) then
         colat0 = rad_per_deg*(90.0 - proj%truelat1)
      ...
      else if (proj%code == PROJ_ROTLL) then
